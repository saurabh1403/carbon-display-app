package com.Tutorial.business
{
	...
	public class NativeCommunicator
	{
		...
		public static function get instance():NativeCommunicator
		{
			if(!_instance)
			{
				_instance = new NativeCommunicator();
			}
			return _instance;
		}
		...
		private static function onOutputData(event:ProgressEvent): void
		{
			...
			ApplicationFacade.getInstance().sendNotification(TutConstants.NativeDataRecievedNotification, data);
					...
					if(mObj != null)
					{
						...
						switch(evName)
						{
							...
							case "getPackageSessionData":
								...
								break;
							case "getContent":
s								ApplicationFacade.getInstance().sendNotification(TutConstants.HIDE_SPINNING_OVERLAY);
